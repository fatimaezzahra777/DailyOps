@php
    $projectNavItems = [
        ['label' => 'Board', 'icon' => 'ti-layout-dashboard', 'route' => 'projects.index'],
        ['label' => 'Table', 'icon' => 'ti-table', 'route' => 'projects.table'],
        ['label' => 'Gantt', 'icon' => 'ti-chart-bar', 'route' => 'projects.gantt'],
        ['label' => 'Calendar', 'icon' => 'ti-calendar', 'route' => 'projects.calendar'],
        ['label' => 'Reports', 'icon' => 'ti-report', 'route' => 'projects.reports'],
    ];
@endphp
...
<aside x-data="{ open: false }" class="hidden w-[230px] shrink-0 flex-col border-r border-black/10 bg-white shadow-[2px_0_12px_rgba(0,0,0,0.06)] sm:flex">
    ...
    <nav class="min-h-0 flex-1 overflow-y-auto py-4">
        ...
        @foreach ($projectNavItems as $item)
            <a href="{{ route($item['route']) }}" class="relative mx-2 mb-1 flex items-center gap-3 rounded-md px-3 py-2 text-[13px] transition {{ request()->routeIs($item['route']) ? 'border border-[#e8007d]/20 bg-[#e8007d]/10 text-[#e8007d] before:absolute before:-left-2 before:top-1/2 before:h-4 before:w-[3px] before:-translate-y-1/2 before:rounded before:bg-[#e8007d]' : 'text-[#555555] hover:bg-[#f4f4f4] hover:text-[#0a0a0a]' }}">
...
<div x-data="{ open: false }" class="sm:hidden">
    ...
    <aside x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full" class="fixed inset-y-0 left-0 z-50 flex w-[82vw] max-w-[300px] flex-col border-r border-black/10 bg-white shadow-xl">
        <div class="flex items-center gap-3 border-b border-black/10 px-5 py-5 pl-16">
            <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
                <span class="flex h-8 w-8 items-center justify-center rounded-[9px] bg-[#e8007d] text-white shadow-[0_0_18px_rgba(232,0,125,0.28)]">
        ...
        <nav class="min-h-0 flex-1 overflow-y-auto py-4">
            ...
            <a href="{{ route('dashboard') }}" class="relative mx-2 mb-1 flex items-center gap-3 rounded-md px-3 py-2.5 text-[13px] transition {{ request()->routeIs('dashboard') ? 'border border-[#e8007d]/20 bg-[#e8007d]/10 text-[#e8007d]' : 'text-[#555555] hover:bg-[#f4f4f4] hover:text-[#0a0a0a]' }}">
                <i class="ti ti-layout-dashboard text-base"></i>
                <span>Dashboard</span>
            </a>

            @if (Auth::user()->role === 'admin')
                <a href="{{ route('users.index') }}" class="relative mx-2 mb-1 flex items-center gap-3 rounded-md px-3 py-2.5 text-[13px] transition {{ request()->routeIs('users.*') ? 'border border-[#e8007d]/20 bg-[#e8007d]/10 text-[#e8007d]' : 'text-[#555555] hover:bg-[#f4f4f4] hover:text-[#0a0a0a]' }}">
                    <i class="ti ti-users text-base"></i>
                    <span>Users</span>
                </a>
            @endif
            ...
            @foreach ($projectNavItems as $item)
                <a href="{{ route($item['route']) }}" class="relative mx-2 mb-1 flex items-center gap-3 rounded-md px-3 py-2.5 text-[13px] transition {{ request()->routeIs($item['route']) ? 'border border-[#e8007d]/20 bg-[#e8007d]/10 text-[#e8007d]' : 'text-[#555555] hover:bg-[#f4f4f4] hover:text-[#0a0a0a]' }}">
                    <i class="ti {{ $item['icon'] }} text-base"></i>
