<x-app-layout>
    <x-slot name="header">
        <h2 class="font-['Syne'] text-base font-bold tracking-wide text-[#0a0a0a]">Edit {{ $user->name }}</h2>
    </x-slot>
        ...
        <div class="mx-auto max-w-5xl rounded-[14px] border border-black/10 bg-white px-6 py-8 shadow-[0_8px_30px_rgba(0,0,0,0.08)] sm:px-10 lg:px-14 lg:py-12">
            <div class="mb-10">
                ...
                <p class="mt-5 text-[15px] text-[#6b7280] sm:text-base">
                    Update login information and role for {{ $user->name }}.
                </p>
            </div>
            ...
            <div>
                <form method="POST" action="{{ route('users.update', $user) }}">
                    @method('PUT')
                    @include('users._form', ['submitLabel' => 'Save'])
                </form>
            </div>
        </div>
    </div>
