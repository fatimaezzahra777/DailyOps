<x-app-layout>
    <x-slot name="header">
        <h2 class="font-['Syne'] text-base font-bold tracking-wide text-[#0a0a0a]">Détails de l’utilisateur</h2>
    </x-slot>
        ...
        <div class="mx-auto max-w-5xl rounded-[14px] border border-black/10 bg-white px-6 py-8 shadow-[0_8px_30px_rgba(0,0,0,0.08)] sm:px-10 lg:px-14 lg:py-12">
            ...
            <section>
                ...
                <dl class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    ...
                    <div class="rounded-[10px] border border-[#dce4ef] bg-[#f8fafc] px-4 py-4">
                        <dt class="text-[12px] font-semibold text-[#6b7280]">E-mail</dt>
                        <dd class="mt-2 break-all text-[15px] font-bold text-[#111827]">{{ $user->email }}</dd>
                    </div>
                    <div class="rounded-[10px] border border-[#dce4ef] bg-[#f8fafc] px-4 py-4">
                        <dt class="text-[12px] font-semibold text-[#6b7280]">Date de naissance</dt>
                        <dd class="mt-2 text-[15px] font-bold text-[#111827]">
                            {{ $user->birth_date?->format('d/m/Y') ?? 'Non renseignée' }}
                        </dd>
                    </div>
                </dl>
            </section>
            ...
            <section class="mt-10">
                ...
                <dl class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    <div class="rounded-[10px] border border-[#dce4ef] bg-[#f8fafc] px-4 py-4">
                        <dt class="text-[12px] font-semibold text-[#6b7280]">Rôle</dt>
                        <dd class="mt-2">
                    ...
                    <div class="rounded-[10px] border border-[#dce4ef] bg-[#f8fafc] px-4 py-4">
                        <dt class="text-[12px] font-semibold text-[#6b7280]">Créé le</dt>
                        <dd class="mt-2 text-[15px] font-bold text-[#111827]">{{ $user->created_at->format('d/m/Y H:i') }}</dd>
                    </div>
                    <div class="rounded-[10px] border border-[#dce4ef] bg-[#f8fafc] px-4 py-4">
                        <dt class="text-[12px] font-semibold text-[#6b7280]">Mis à jour le</dt>
            ...
            <section class="mt-10">
                ...
                @php
                    $projectCard = function ($project) {
                        ...
                        return [
                            ...
                            'statusLabel' => ['pending' => 'En attente', 'in_progress' => 'En cours', 'completed' => 'Terminé'][$project->status] ?? $project->status,
                ...
                @if ($user->isAdmin())
                    <div class="grid gap-4 md:grid-cols-2">
                            ...
                            <article class="rounded-[10px] border border-[#dce4ef] bg-[#f8fafc] p-4">
                                <div class="flex items-start justify-between gap-3">
                                    <div>
                                        ...
                                        <p class="mt-1 text-xs text-[#6b7280]">Responsable : {{ $project->manager?->name ?? 'Non assigné' }}</p>
                                    </div>
                        ...
                        @empty
                            <p class="rounded-[10px] border border-dashed border-[#dce4ef] bg-[#f8fafc] p-5 text-sm text-[#6b7280]">Aucun projet trouvé.</p>
                        @endforelse
                    </div>
                @else
                    <div class="grid gap-6 lg:grid-cols-2">
                        <div>
                            <h3 class="mb-3 text-sm font-extrabold text-[#111827]">Projets créés ou gérés</h3>
                            <div class="space-y-3">
                                ...
                                @empty
                                    <p class="rounded-[10px] border border-dashed border-[#dce4ef] bg-[#f8fafc] p-5 text-sm text-[#6b7280]">Aucun projet géré.</p>
                                @endforelse
                            </div>
                        </div>

                        <div>
                            <h3 class="mb-3 text-sm font-extrabold text-[#111827]">Projets assignés ou partagés</h3>
                            <div class="space-y-3">
                                ...
                                @empty
                                    <p class="rounded-[10px] border border-dashed border-[#dce4ef] bg-[#f8fafc] p-5 text-sm text-[#6b7280]">Aucun projet assigné.</p>
                                @endforelse
                            </div>
                        </div>
                    </div>
